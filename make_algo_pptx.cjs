...
// ============================================================
{
  ...
  s.addText('Cold Start: What Happens With No Ratings?', {
    x: 0.5, y: 0.22, w: 9, h: 0.55, fontSize: 28, fontFace: 'Calibri', bold: true, color: C.slate, align: 'left', margin: 0
  })
  s.addShape(pres.shapes.RECTANGLE, { x: 0.5, y: 0.79, w: 9, h: 0.04, fill: { color: C.purple }, line: { color: C.purple } })

  // Code trace box
  s.addShape(pres.shapes.RECTANGLE, { x: 0.4, y: 0.95, w: 9.2, h: 1.55, fill: { color: C.navy }, line: { color: C.navy }, shadow: makeShadow() })
  s.addText([
    { text: '# student has no ratings → student_id not in ratings dict\n', options: { color: '506070' } },
    { text: 'cf_score = compute_cf_score(ratings, student_profile.id, tutor_id)\n', options: { color: C.white } },
    { text: '# student_id not in ratings → returns ', options: { color: '506070' } },
    { text: 'None\n', options: { color: C.amber } },
    { text: '\nif cf_score is None:\n', options: { color: C.white } },
    { text: '    cf_score = ', options: { color: C.white } },
    { text: '0', options: { bold: true, color: C.amber } },
    { text: '   # hybrid.py line 28–29', options: { color: '506070' } },
  ], { x: 0.6, y: 1.0, w: 8.8, h: 1.42, fontSize: 11, fontFace: 'Consolas', margin: 0 })

  // Formula collapse box
  s.addShape(pres.shapes.RECTANGLE, { x: 0.4, y: 2.65, w: 9.2, h: 1.0, fill: { color: '1A3050' }, line: { color: C.purple, width: 2 }, shadow: makeShadow() })
  s.addText([
    { text: 'hybrid_score', options: { bold: true, color: C.amber } },
    { text: '  =  0.7 × cbf_score  +  0.3 × (', options: { color: 'A0B4CC' } },
    { text: '0', options: { bold: true, color: C.amber } },
    { text: ' ÷ 5)  =  ', options: { color: 'A0B4CC' } },
    { text: '0.7 × cbf_score', options: { bold: true, color: C.tealLight } },
  ], { x: 0.4, y: 2.65, w: 9.2, h: 0.52, fontSize: 18, fontFace: 'Consolas', align: 'center', valign: 'bottom', margin: 0 })
  s.addText('CF term = 0 · ranking is 100% CBF for new students', {
    x: 0.4, y: 3.18, w: 9.2, h: 0.38, fontSize: 13, fontFace: 'Calibri', italic: true, color: C.slateLight, align: 'center', margin: 0
  })

  // Two takeaway cards
  const cards = [
    {
      title: 'CBF handles cold start', color: C.teal,
      body: 'CBF still runs fully — it uses the student\'s course, year level, and subjects from their Preference model. No prior ratings needed at all.',
      x: 0.4,
    },
    {
      title: 'CF activates after first rating', color: C.blue,
      body: 'As soon as a student leaves their first rating, they enter the rating matrix and Pearson neighbors can be found. CF begins contributing to their hybrid score.',
      x: 5.1,
    },
  ]
  cards.forEach(c => {
    s.addShape(pres.shapes.RECTANGLE, { x: c.x, y: 3.72, w: 4.5, h: 1.6, fill: { color: C.white }, line: { color: c.color, width: 1.5 }, shadow: makeShadow() })
    s.addShape(pres.shapes.RECTANGLE, { x: c.x, y: 3.72, w: 4.5, h: 0.38, fill: { color: c.color }, line: { color: c.color } })
    s.addText(c.title, { x: c.x, y: 3.72, w: 4.5, h: 0.38, fontSize: 13, bold: true, color: C.white, align: 'center', valign: 'middle', margin: 0 })
    s.addText(c.body, { x: c.x + 0.15, y: 4.15, w: 4.2, h: 1.1, fontSize: 11, fontFace: 'Calibri', color: C.slate, margin: 0 })
  })

  s.addShape(pres.shapes.RECTANGLE, { x: 0, y: 5.38, w: 10, h: 0.24, fill: { color: C.purple }, line: { color: C.purple } })
  s.addText('hybrid.py lines 28–29  ·  CF = 0 is not an error — it is the intended cold-start behaviour', {
    x: 0, y: 5.38, w: 10, h: 0.24, fontSize: 10, fontFace: 'Calibri', color: C.white, align: 'center', valign: 'middle', margin: 0
  })
}

// ============================================================
// SLIDE 13: WHERE IT'S APPLIED
// ============================================================
{
  const s = pres.addSlide()
  s.background = { color: C.offWhite }
