...
|------|------|--------|---------|
| 2026-08-05 | [Tutor dashboard simplification and booking pagination](2026-08-05-tutor-dashboard-simplification.md) | Approved | Reframes `TutorDashboard.vue` from a metrics report into a work queue: six metric cards (two carrying fabricated sparkline data) cut to a three-value compact strip, booking cards rebuilt as a date-grouped schedule with client-side day-packed pagination, and a `Needs Attention` rail (Requests capped at 5 + expander, Payments uncapped) in a right-rail `sidebar-and-main` layout per the approved mockup ([mockup](../mockups/2026-08-05-tutor-dashboard-simplification.html)); status routing keys off `raw_status` except the Payments bucket, since `get_display_status` splits `Confirmed` into three clock-dependent states; no backend change; not yet implemented |
