...
is Approved — grilled end-to-end (7 decisions) from a feature request that started as "course
dropdown at sign-up" and was corrected mid-interview to `Subjects`, not `Course`. Tutee
subject-selection folds into the existing `PreferenceSetup.vue` step rather than a new dedicated
step (ui-preview session settled this); `Subjects` gets a new admin-editable `description` field
shown in the shared `SubjectTaxonomyPicker`. Commission disclosure deliberately deviates from
ADR-0007's UI-only pattern — acceptance is persisted (`Tutor.commission_terms_accepted_at`),
disclosed inline at the `hourly_rate` onboarding field, and retroactively forced on already-onboarded
(including seeded) tutors via the router guard, recorded as
[ADR-0010](../adr/0010-persisted-commission-disclosure-acceptance.md). Not yet implemented —
...
|------|------|--------|---------|
| 2026-08-07 | [Subject Selection Onboarding + Commission Disclosure](2026-08-07-subject-selection-and-commission-disclosure.md) | Approved | Grilled end-to-end (7 decisions) from a request that started as "course dropdown at sign-up," corrected mid-interview to `Subjects`. Tutee subject-selection folds into `PreferenceSetup.vue` rather than a new step ([mockup](../mockups/2026-08-07-tutee-subject-selection.html)); `Subjects` gains an admin-editable `description` field surfaced in the shared `SubjectTaxonomyPicker`. Commission disclosure deliberately deviates from ADR-0007's UI-only pattern — persisted acceptance (`Tutor.commission_terms_accepted_at`), disclosed at the `hourly_rate` onboarding field, retroactively forced on existing/seeded tutors via the router guard — [ADR-0010](../adr/0010-persisted-commission-disclosure-acceptance.md). Not yet implemented |
