...
dispatched to Codex CLI on a dedicated `feat/tutor-onboarding-verification-redesign` branch (skips
the long-running full backend test suite per instruction; full-suite verification deferred to
...
|------|------|--------|---------|
| 2026-07-13 | [Tutor onboarding & verification redesign](2026-07-13-tutor-onboarding-verification-redesign.md) | In Progress | Removes the tutor route-lockout for a tutee-style search-visibility gate, folds verification into one guided onboarding sequence with a skip option, and lets tutors propose subjects missing from the catalog for admin review alongside their application; both onboarding screens mocked and decided; dispatched to Codex via `docs/briefs/2026-07-14-tutor-onboarding-verification-redesign.md` |
