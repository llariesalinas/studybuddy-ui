...
const categories = ['Mathematics & Data Sciences', 'Natural Sciences', 'Technology & Computer Science', 'Business, Finance & Economics', 'Humanities & Social Sciences', 'Hobbies & Arts']
...
const form = reactive({ subject_name: '', department: '', category: '' })
const filteredSubjects = computed(() => { const query = search.value.toLowerCase(); return catalogStore.courseCatalog.filter((subject) => !query || [subject.subject_name, subject.department, subject.category].filter(Boolean).some((value) => String(value).toLowerCase().includes(query))) })
function resetForm() { Object.assign(form, { subject_name: '', department: '', category: '' }); editingCode.value = null }
function startCreate() { resetForm(); showForm.value = true }
function startEdit(subject) { editingCode.value = subject.subject_code; Object.assign(form, { subject_name: subject.subject_name, department: subject.department, category: subject.category }); showForm.value = true }
